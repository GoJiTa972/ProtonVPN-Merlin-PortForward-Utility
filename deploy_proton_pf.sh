...
while true; do
    ...
    if [ -z "$CURRENT_PORT" ]; then
        ...
        if [ "$CURRENT_PORT" = "$OLD_PORT" ]; then
            ...
            logger -t "PortForward" "[Instance $INSTANCE_ID] Port changed from ${OLD_PORT:-None} to $CURRENT_PORT. Waiting for BiglyBT... (URL: $RPC_URL, User: $RPC_USER)"
            ...
            while [ $ATTEMPT -lt $MAX_ATTEMPTS ]; do
                ...
                if [ -n "$TR_SESSION" ]; then
                    ...
                    logger -t "PortForward" "[Instance $INSTANCE_ID] BiglyBT API (HTTP: $HTTP_CODE) limits applied | Firewall routed port $CURRENT_PORT to $PC_IP. $RESPONSE_LOG"
                    SUCCESS=1
                    break
                else
                    if [ "$PF_LOG_LEVEL" = "DEBUG" ]; then
            ...
            if [ $SUCCESS -eq 0 ]; then
                logger -t "PortForward" "[Instance $INSTANCE_ID] Gave up waiting for BiglyBT after $MAX_ATTEMPTS attempts. Will try again on next daemon cycle."
            fi
        fi
    fi
